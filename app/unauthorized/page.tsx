import Link from "next/link";
import { ShieldAlert } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { buttonVariants } from "@/components/ui/button";

export default function UnauthorizedPage() {
  return (
    <main className="grid min-h-[calc(100vh-4rem)] place-items-center bg-[var(--background)] px-5 py-10 sm:px-8">
      <Card className="w-full max-w-xl">
        <CardHeader>
          <div className="grid size-11 place-items-center rounded-md bg-[var(--surface-strong)] text-[#9d3f29]">
            <ShieldAlert className="size-5" aria-hidden="true" />
          </div>
          <CardTitle className="mt-5 text-3xl">Not authorized</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm leading-6 text-[var(--muted)]">
            Your account does not have access to this admin area. Admin role
            checks are enforced on the server before protected data is loaded.
          </p>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <Link className={buttonVariants()} href="/dashboard">
              Back to dashboard
            </Link>
            <Link
              className={buttonVariants({ variant: "secondary" })}
              href="/issues"
            >
              Browse public issues
            </Link>
          </div>
        </CardContent>
      </Card>
    </main>
  );
}
